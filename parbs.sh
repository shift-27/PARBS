#!/bin/sh
# Pigeon's Auto Rice Boostrapping Script
# this is literally a fork of LARBS by luke @ larbs.xyz
# by pigeon <pigeongode@gmail.com>
# License: GNU GPLv3
...
welcomemsg() { \
	dialog --title "Welcome!" --msgbox "Welcome Pigeon's Auto-Rice Bootstrapping Script!\\n\\nThis script will automatically install a fully-featured Linux desktop, which I use as my main machine.\\n\\n-Pigeon" 10 60

	dialog --colors --title "Important Note!" --yes-label "All ready!" --no-label "Return..." --yesno "This will not work on Arch!!. This will only work on Artix-openrc. Be sure the computer you are using has current pacman updates and refreshed Arch keyrings.\\n\\nIf it does not, the installation of some programs might fail." 8 70
	}
...
gitmakeinstall() {
	...
	dialog --title "PARBS Installation" --infobox "Installing \`$progname\` ($n of $total) via \`git\` and \`make\`. $(basename "$1") $2" 5 70
...
pipinstall() { \
	dialog --title "PARBS Installation" --infobox "Installing the Python package \`$1\` ($n of $total). $1 $2" 5 70
...
for x in curl base-devel git ntp zsh; do
	dialog --title "PARBS Installation" --infobox "Installing \`$x\` which is required to install and configure other programs." 5 70
	installpkg "$x"
done

dialog --title "PARBS Installation" --infobox "Synchronizing system time to ensure successful and secure installation of software..." 4 70
...
dialog --title "PARBS Installation" --infobox "Finally, installing \`libxft-bgra\` to enable color emoji in suckless software without crashes." 5 70
